...
class SubjectResourceIT {
    ...
    public static Subject createEntity(EntityManager em) {
        Subject subject = new Subject().nameMat(DEFAULT_NAME_MAT).coefMat(DEFAULT_COEF_MAT);
        // Add required entity
        Degree degree;
        if (TestUtil.findAll(em, Degree.class).isEmpty()) {
            degree = DegreeResourceIT.createEntity(em);
            em.persist(degree);
            em.flush();
        } else {
            degree = TestUtil.findAll(em, Degree.class).get(0);
        }
    ...
    public static Subject createUpdatedEntity(EntityManager em) {
        Subject subject = new Subject().nameMat(UPDATED_NAME_MAT).coefMat(UPDATED_COEF_MAT);
        // Add required entity
        Degree degree;
        if (TestUtil.findAll(em, Degree.class).isEmpty()) {
            degree = DegreeResourceIT.createUpdatedEntity(em);
            em.persist(degree);
            em.flush();
        } else {
            degree = TestUtil.findAll(em, Degree.class).get(0);
    ...
    void createSubject() throws Exception {
        ...
        assertThat(testSubject.getNameMat()).isEqualTo(DEFAULT_NAME_MAT);
        assertThat(testSubject.getCoefMat()).isEqualTo(DEFAULT_COEF_MAT);
    }
    ...
    void putNewSubject() throws Exception {
        ...
        updatedSubject.nameMat(UPDATED_NAME_MAT).coefMat(UPDATED_COEF_MAT);
        ...
        Subject testSubject = subjectList.get(subjectList.size() - 1);
        assertThat(testSubject.getNameMat()).isEqualTo(UPDATED_NAME_MAT);
        assertThat(testSubject.getCoefMat()).isEqualTo(UPDATED_COEF_MAT);
    }
    ...
    void partialUpdateSubjectWithPatch() throws Exception {
        ...
        partialUpdatedSubject.nameMat(UPDATED_NAME_MAT);
    ...
    void fullUpdateSubjectWithPatch() throws Exception {
        ...
        partialUpdatedSubject.nameMat(UPDATED_NAME_MAT).coefMat(UPDATED_COEF_MAT);
